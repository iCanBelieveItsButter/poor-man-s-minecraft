...
def multiply_matrices(x_matrix, y_matrix):
    result_matrix = [[0.0]*4 for _ in range(4)]
    for i in range(4):
        for j in range(4):
            result_matrix[i][j] = (
...
class Matrix:
    def __init__(self, base=None):
        # Create clean 4x4 matrix
        self.clean_matrix = [[0.0]*4 for _ in range(4)]
        self.identity_matrix = copy_matrix(self.clean_matrix)
        for i in range(4):
            self.identity_matrix[i][i] = 1.0
        ...
        if isinstance(base, Matrix):
            ...
            self.data = copy_matrix(self.clean_matrix)

    def load_identity(self):
        self.data = copy_matrix(self.identity_matrix)
    ...
    def translate(self, x, y, z):
        for i in range(4):
            self.data[3][i] += (self.data[0][i] * x + self.data[1][i] * y + self.data[2][i] * z)
    ...
    def rotate(self, angle, x, y, z):
        ...
        rotation_matrix = copy_matrix(self.clean_matrix)
    ...
    def frustum(self, left, right, bottom, top, near, far):
        ...
        frustum_matrix = copy_matrix(self.clean_matrix)
    ...
    def orthographic(self, left, right, bottom, top, near, far):
        ...
        ortho_matrix = copy_matrix(self.identity_matrix)
        ortho_matrix[0][0] = 2.0 / deltax
        ortho_matrix[3][0] = -(right + left) / deltax
        ortho_matrix[1][1] = 2.0 / deltay
        ortho_matrix[3][1] = -(top + bottom) / deltay
        ortho_matrix[2][2] = 2.0 / deltaz
        ortho_matrix[3][2] = -(near + far) / deltaz
